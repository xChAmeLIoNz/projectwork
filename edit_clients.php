...
try {
    ...
    $stmt = $mysqli->prepare("UPDATE pw_iseppe_users SET first_name=?,
    last_name=?, dob=?, email=?, telephone=? WHERE id=?");
    //STAGE 2: bind and execute
    $stmt->bind_param("sssssi", $first_name, $last_name, $dob, $email, $number, $id);
    $stmt->execute();
